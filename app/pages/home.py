...
def _run_full_pipeline(ticker: str) -> int:
    from src.pipeline_runner import run_ingest_pipeline, run_embed_pipeline, run_analysis_pipeline
    ...
    if needs_ingest:
        ...
        if n == 0:
            st.markdown(
                ...
                f'This usually means the ticker is not listed on a US exchange.</div>'
                ...
                f'Try: AAPL · MSFT · NVDA · TSLA · JPM · META · AMZN'
                f'</div></div>',
                unsafe_allow_html=True,
            )
            st.stop()
...
for col, (title, color, *items) in zip(arch_cols, [
    ("01 — Ingest",   "var(--gold)",  "SEC EDGAR 10-K/10-Q/8-K", "Earnings call transcripts", "Analyst consensus · yfinance", "XBRL financial data"),
    ("02 — Analyse",  "var(--taupe)", "FinBERT sentiment scoring", "NLI contradiction detection", "NER guidance extraction", "Accruals quality ratio"),
    ("03 — Signal",   "var(--green)", "Composite quality score [-1,+1]", "LangGraph multi-hop RAG", "Groq / Gemini synthesis", "RAGAS-evaluated answers"),
]):
    with col:
